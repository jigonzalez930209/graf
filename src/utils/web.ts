...
const readAllWebProcess = async (allFiles: { content: string; name: string }[]) => {
    ...
    allFiles
      ...
      .map(async (file, i) => {
        if (fileType(file.name) === 'csv') {
          ...
          const columns: string[] = invariableContent[selectedInvariableContentIndex] as string[]
          const value = {
            content,
            name: file.name,
            invariableContent,
...
const readAllFilesUsingWebProcess = async (files: FileList) => {
  const response = await readAllFilesUsingJS(Object.keys(files).map(i => files[i]))
...
export { readAllFilesUsingWebProcess, readAllFilesUsingJS, readAllWebProcess, readFileContentsUsingJS }
